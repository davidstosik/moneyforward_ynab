...
steps = ARGV
steps = %w(download convert csv-export ynab-import) if steps.empty?
puts "Running steps: #{steps.join(", ")}"

# FIXME use temporary folder unless in "debug" mode?
save_path = File.join(__dir__, "in", script_time.strftime("%Y%m%d-%H%M%S"))
latest_symlink = File.join(__dir__, "in", "latest")

config = YAML.load_file(File.join(__dir__, "config.yml")).values.first
...
if steps.include?("convert")
  ...
  #  output_path = File.join(__dir__, "out", script_time.strftime("%Y%m%d-%H%M%S"))
